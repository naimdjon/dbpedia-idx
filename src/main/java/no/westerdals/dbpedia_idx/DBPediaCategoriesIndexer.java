...
public class DBPediaCategoriesIndexer {
    final Pattern categoryPattern = Pattern.compile("\\s*?<http://dbpedia.org/resource/(.*?)>\\s+<http://purl.org/dc/terms/subject>\\s+<http://dbpedia.org/resource/Category:(.*?)>.*");
    private final String inputFile;
    private static long batchSize = 12_000L;

    public DBPediaCategoriesIndexer(final String inputFile) {
    ...
    private Consumer<String> categoryConsumer(final TripleProcessor tripleProcessor) {
        return line -> {
            ...
            if (matcher.find()) {
                tripleProcessor.process(new Triple(matcher.group(1), "subject", matcher.group(2)));
            } else {
